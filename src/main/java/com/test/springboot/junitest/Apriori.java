...
public class Apriori {
					...
					for (int j = 0; j < key_allSubset.size(); j++) {
						...
						if (0 < item_post.size()
							...
							if (confident > min_confident) {// 如果事件的置信度大于最小置信度
								System.out.println(item_pre + "==>" + item_post+"  置信度："+confident);// 则是一个关联事件
							}
						}
	...
	public static void main(String[] args) {
		...
		init();
		System.out.println("*************事务数据库******************");
		System.out.println(D); 
		System.out.println("*************项目集以及支持度******************");
		System.out.println(C); 
		System.out.println("*************剔除未达标项目集-候选集******************");
		System.out.println(L);
		L = iteration(C, L);
		System.out.println("*************候选集所有排列组合******************");
		System.out.println(L_ALL);
		System.out.println("*************最终频繁集******************");
		System.out.println(L); 
